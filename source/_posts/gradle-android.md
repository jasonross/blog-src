...
---
随着Android Studio替代Eclipse成为Android官方推荐的IDE，Gradle作为新一代Android应用构建工具，逐渐登上历史舞台。Ant、Maven、Gradle，项目构建工具，你方唱罢我登场。

最近在忙团队并行开发的事情，主要是将各个团队的代码分库，一方面可以降低耦合，为后面模块插件化做铺垫，另一方面通过AAR二进制编译，加快完整编译速度，过程中遇到了很多问题，通过了解Gradle进行了逐一化解。

Gradle不愧为构建神器，灵活强大，但感觉学习曲线比较陡峭，[Gradle User Guide](https://docs.gradle.org/current/userguide/userguide.html)内容很多，但有点太多了，多的你看不完，[Gradle Plugin User Guide](http://tools.android.com/tech-docs/new-build-system/user-guide)一篇文章主要讲了Android相关的配置，看完可能感觉马马虎虎会用，但到了修改一些构建流程的时候还是不知所以。经过一段时间的摸索，我觉得在Android Studio中用好Gradle，你要做到以下三点：

* 了解 `Groovy` 基本语法
* 粗读 `Gradle User Guide` 和 `Gradle Plugin User Guide`
* 实战

涉及到的知识点和内容比较多，我不会一一讲解，本文主要会解答自我学习过程中的一些疑问，讲解一些概念和实战经验，过程中会推荐一些有质量的博客文章。
...
一个Task的inputs和outputs可以是一个或多个文件，可以是文件夹，还可以是Project的某个Property，甚至可以是某个闭包所定义的条件。自定义task默认每次执行，我们可以通过指定inputs和outputs，可以做到增量构建的效果。
...
你怎么知道这个任务名称的？Android Studio中的Gradle Console在执行gradle任务的时候会有输出，可以仔细观察一下。

hello任务定义的这么复杂干啥？我直接就叫hello不行吗?不行，each就是遍历variants，如果每个都叫hello，岂不是傻傻分不清楚，加上variant的name做后缀，才有任务的区分。
...
我靠，仓库竟然直接在Android Studio应用内部，所以说你去掉buildscript的jcenter()完全没有关系啊，下面还有更爽的，我们知道有依赖传递，`gradle`依赖`gradle-core`，`gradle-core`依赖`lint`，`lint`依赖`lint-checks`，`lint-checks`最后依赖到了`asm`，并且这个依赖配置会传到所有工程，所以你要引用asm相关的类，不用设置classpath，直接import就可以了。你怎么知道的啊？看上面m2repository目录中对应的pom文件。

到asm怎么了？[ASM](http://asm.ow2.org/)又是个比较刁的东西，可以直接用来操纵Java字节码，达到动态更改class文件的效果。可以用ASM[面向切面编程](http://developer.51cto.com/art/201309/410861_all.htm)，达到解耦效果。[Android DEX自动拆包及动态加载简介](http://tech.meituan.com/mt-android-auto-split-dex.html)中提到的class依赖分析和R常量替换的脚本都可以用ASM来搞。
